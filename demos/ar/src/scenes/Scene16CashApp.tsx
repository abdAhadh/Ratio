...
function ICICILogo() {
  return (
    <div className="bg-white rounded-md px-2 py-1 flex items-center">
      <img src={`${base}logos/icici.svg`} alt="ICICI Bank" style={{ height: 24, width: 'auto' }} />
    </div>
  );
}
...
function SlackLogo({ size = 18 }: { size?: number }) {
  return (
    <img src={`${base}logos/slack.svg`} alt="Slack" width={size} height={size} style={{ objectFit: 'contain' }} />
  );
}
