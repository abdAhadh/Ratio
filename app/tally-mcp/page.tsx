import type { Metadata } from "next";
import { SiteNav } from "@/components/site-nav";
import { SiteFooter } from "@/components/site-footer";
import { UnicornShader } from "@/components/hero/unicorn-shader";
import styles from "./tally-mcp.module.css";

/**
 * /tally-mcp — "Free Tally MCP / Connector"
 *
 * Page promoting the free Tally Prime → Claude / ChatGPT MCP server guide.
 * Carried over from the previous tryratio.io site (UAE/India brand) and
 * re-skinned into the dark Ratio template: same SiteNav + SiteFooter as
 * the rest of the site, page-specific styling lives in tally-mcp.module.css.
 *
 * Linked from the COMPANY column in the footer only — never in the top nav,
 * matching the original page's discovery model.
 */

export const metadata: Metadata = {
  title: "Connect Tally Prime to Claude & ChatGPT · Ratio",
  description:
    "A free, copy-paste MCP server guide. Go from zero to AI-powered finance ops in under 30 minutes. Works with Tally Prime on desktop and on cloud.",
  openGraph: {
    title: "Connect Tally Prime to Claude & ChatGPT · Ratio",
    description:
      "A free, copy-paste MCP server guide. Go from zero to AI-powered finance ops in under 30 minutes.",
    type: "website",
  },
};

/* Claude logomark — single-path glyph from the official wordmark.
   Uses currentColor so the parent's color (white) drives the fill on
   the dark background. */
const ClaudeLogo = () => (
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path
      d="M4.709 15.955l4.72-2.647.08-.23-.08-.128H9.2l-.79-.048-2.698-.073-2.339-.097-2.266-.122-.571-.121L0 11.784l.055-.352.48-.321.686.06 1.52.103 2.278.158 1.652.097 2.449.255h.389l.055-.157-.134-.098-.103-.097-2.358-1.596-2.552-1.688-1.336-.972-.724-.491-.364-.462-.158-1.008.656-.722.881.06.225.061.893.686 1.908 1.476 2.491 1.833.365.304.145-.103.019-.073-.164-.274-1.355-2.446-1.446-2.49-.644-1.032-.17-.619a2.97 2.97 0 01-.104-.729L6.283.134 6.696 0l.996.134.42.364.62 1.414 1.002 2.229 1.555 3.03.456.898.243.832.091.255h.158V9.01l.128-1.706.237-2.095.23-2.695.08-.76.376-.91.747-.492.584.28.48.685-.067.444-.286 1.851-.559 2.903-.364 1.942h.212l.243-.242.985-1.306 1.652-2.064.73-.82.85-.904.547-.431h1.033l.76 1.129-.34 1.166-1.064 1.347-.881 1.142-1.264 1.7-.79 1.36.073.11.188-.02 2.856-.606 1.543-.28 1.841-.315.833.388.091.395-.328.807-1.969.486-2.309.462-3.439.813-.042.03.049.061 1.549.146.662.036h1.622l3.02.225.79.522.474.638-.079.485-1.215.62-1.64-.389-3.829-.91-1.312-.329h-.182v.11l1.093 1.068 2.006 1.81 2.509 2.33.127.578-.322.455-.34-.049-2.205-1.657-.851-.747-1.926-1.62h-.128v.17l.444.649 2.345 3.521.122 1.08-.17.353-.608.213-.668-.122-1.374-1.925-1.415-2.167-1.143-1.943-.14.08-.674 7.254-.316.37-.729.28-.607-.461-.322-.747.322-1.476.389-1.924.315-1.53.286-1.9.17-.632-.012-.042-.14.018-1.434 1.967-2.18 2.945-1.726 1.845-.414.164-.717-.37.067-.662.401-.589 2.388-3.036 1.44-1.882.93-1.086-.006-.158h-.055L4.132 18.56l-1.13.146-.487-.456.061-.746.231-.243 1.908-1.312-.006.006z"
      fill="currentColor"
      fillRule="nonzero"
    />
  </svg>
);

/* ChatGPT mark — inline SVG already using currentColor for the path. */
const ChatGptLogo = () => (
  <svg
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="-0.171 0.482 41.142 40.034"
    aria-hidden="true"
  >
    <path
      d="M37.532 16.87a9.963 9.963 0 0 0-.856-8.184 10.078 10.078 0 0 0-10.855-4.835A9.964 9.964 0 0 0 18.306.5a10.079 10.079 0 0 0-9.614 6.977 9.967 9.967 0 0 0-6.664 4.834 10.08 10.08 0 0 0 1.24 11.817 9.965 9.965 0 0 0 .856 8.185 10.079 10.079 0 0 0 10.855 4.835 9.965 9.965 0 0 0 7.516 3.35 10.078 10.078 0 0 0 9.617-6.981 9.967 9.967 0 0 0 6.663-4.834 10.079 10.079 0 0 0-1.243-11.813zM22.498 37.886a7.474 7.474 0 0 1-4.799-1.735c.061-.033.168-.091.237-.134l7.964-4.6a1.294 1.294 0 0 0 .655-1.134V19.054l3.366 1.944a.12.12 0 0 1 .066.092v9.299a7.505 7.505 0 0 1-7.49 7.496zM6.392 31.006a7.471 7.471 0 0 1-.894-5.023c.06.036.162.099.237.141l7.964 4.6a1.297 1.297 0 0 0 1.308 0l9.724-5.614v3.888a.12.12 0 0 1-.048.103l-8.051 4.649a7.504 7.504 0 0 1-10.24-2.744zM4.297 13.62A7.469 7.469 0 0 1 8.2 10.333c0 .068-.004.19-.004.274v9.201a1.294 1.294 0 0 0 .654 1.132l9.723 5.614-3.366 1.944a.12.12 0 0 1-.114.01L7.04 23.856a7.504 7.504 0 0 1-2.743-10.237zm27.658 6.437l-9.724-5.615 3.367-1.943a.121.121 0 0 1 .113-.01l8.052 4.648a7.498 7.498 0 0 1-1.158 13.528v-9.476a1.293 1.293 0 0 0-.65-1.132zm3.35-5.043c-.059-.037-.162-.099-.236-.141l-7.965-4.6a1.298 1.298 0 0 0-1.308 0l-9.723 5.614v-3.888a.12.12 0 0 1 .048-.103l8.05-4.645a7.497 7.497 0 0 1 11.135 7.763zm-21.063 6.929l-3.367-1.944a.12.12 0 0 1-.065-.092v-9.299a7.497 7.497 0 0 1 12.293-5.756 6.94 6.94 0 0 0-.236.134l-7.965 4.6a1.294 1.294 0 0 0-.654 1.132l-.006 11.225zm1.829-3.943l4.33-2.501 4.332 2.5v5l-4.331 2.5-4.331-2.5V18z"
      fill="currentColor"
    />
  </svg>
);

/* Tally Prime wordmark + flower glyph — full vector, original 800×339
   viewBox preserved so the proportions stay correct. Filtered to white
   via .logoWhite (brightness-0 + invert) for dark-theme consistency. */
const TallyPrimeLogo = () => (
  <svg
    height="48"
    viewBox="99 144 800 339"
    xmlns="http://www.w3.org/2000/svg"
    aria-label="Tally Prime"
    className={styles.logoWhite}
  >
    <path d="m139 424.8h4.6c3.9 0 6.5 1.9 6.5 5.9s-2 5.9-6.5 5.9h-4.6zm0 35.8v-16.9h3.3c10.4 0 15-4.6 15-12.4s-4.6-12.4-15-12.4h-11.8v41.7zm42.9-30.6c0-4.6 2-5.9 5.2-5.9s5.2 1.3 5.2 5.9v17c0 4.5-2 5.9-5.2 5.9s-5.2-1.3-5.2-5.9zm-7.8 16.3c0 10.4 5.9 14.3 13.7 14.3 8.5 0 13.7-3.9 13.7-14.3v-15c0-10.4-5.9-13.7-13.7-13.7s-13.7 3.9-13.7 13.7zm51.5 14.3h7.8l7.1-28.6 5.9 28.6h7.8l9.8-41.6h-7.8l-5.9 26.7-5.9-26.7h-8.4l-5.9 26.7-5.9-26.7h-7.8zm77.4 0v-7.1h-14.9v-11.1h11v-7.2h-11v-9.7h14.9v-6.5h-22.8v41.6zm26.1-35.8h4.6c3.9 0 5.9 2 5.9 5.9s-2 5.9-5.9 5.9h-4.6zm20.2 35.8-7.8-18.9c4.6-2 6.5-5.2 6.5-10.4 0-7.8-4.6-12.4-15-12.4h-12.3v41.7h8.5v-16.9h3.9l7.2 16.9zm51.4-30.6c0-4.6 2-5.9 5.2-5.9s5.9 1.3 5.9 5.9v17c0 4.5-2 5.9-5.9 5.9-3.3 0-5.2-1.3-5.2-5.9zm-8.5 16.3c0 10.4 5.9 14.3 13.7 14.3 8.5 0 13.7-3.9 13.7-14.3v-15c0-10.4-5.9-13.7-13.7-13.7s-13.7 3.9-13.7 13.7zm53.4 14.3v-18.2h10.4v-7.2h-10.4v-9.7h13.7v-6.5h-22.2v41.6zm82.7-33.2c-1.3-5.9-5.9-10.4-13.7-10.4-8.5 0-13 4.6-13 11.7 0 11.7 18.2 11.7 18.2 19.5 0 3.2-2 5.2-5.2 5.2-2.6 0-5.9-2-6.5-6.5l-7.2 1.3c2 8.5 7.2 11.7 14.3 11.7 8.5 0 13.7-4.6 13.7-11.7 0-13-18.2-13.7-18.2-19.5 0-3.3 1.3-4.6 4.6-4.6 2.6 0 4.6 2 5.9 5.9zm25.4 33.2v-41.6h-8.5v41.6zm24.1 0v-33.2l9.1 33.2h6.5l8.5-33.2v33.2h7.1v-41.6h-11l-7.8 26.7-7.9-26.7h-11.7v41.6zm56-35.8h4.6c3.9 0 6.5 1.9 6.5 5.9s-2 5.9-6.5 5.9h-4.6zm0 35.8v-16.9h3.9c10.4 0 15-4.6 15-12.4s-5.2-12.4-15-12.4h-11.7v41.7zm54 0v-7.1h-13v-34.5h-8.5v41.6zm23.4 0v-41.6h-8.4v41.6zm23.5-30.6c0-4.6 2-5.9 5.2-5.9s5.9 1.3 5.9 5.9v1.3h8.5c0-10.4-5.9-13.7-13.7-13.7s-13.7 3.9-13.7 13.7v15c0 10.4 5.9 14.3 13.7 14.3 8.5 0 13.7-3.9 13.7-14.3v-2h-8.5v2.6c0 4.5-2 5.9-5.9 5.9-3.2 0-5.2-1.3-5.2-5.9zm45.6 30.6v-41.6h-7.8v41.6zm15.6-41.6v6.5h9.8v35.1h7.8v-35.1h9.7v-6.5zm48.2 0h-8.5l11.7 24.7v16.9h8.5v-16.9l11.7-24.7h-8.5l-7.1 16.2z" />
    <path d="m429.3 282.9c5.9-3.9 18.2-18.9 18.9-20.2s3.2-3.9.6-6.5-5.2-1.3-8.5.6c-3.2 1.3-16.9 11.7-22.1 16.3-5.2 3.9-18.2 15.6-18.2 15.6s-7.2 5.9-4.6 9.1 5.2 2 8.5.7c3.9-1.3 19.6-11.7 25.4-15.6zm289.1 73.6c2 .6 6.5 2 6.5 6.5 0 0-4.6 9.1-5.9 9.8-2 .6-5.2.6-11.7-2s-38.4-10.4-46.9-12.4-27.3-5.2-27.3-5.2-2.6-1.3-6.5 4.6c-3.9 6.5-16.3 24.1-16.3 24.1-.4 1.5-1.7 2.6-3.2 2.6-2.6 0-12.4-.7-16.3-2.6s-5.2-2-2.6-5.9 11.1-21.5 15-24.7c3.2-3.2-2.6-2.6-4.6-3.2s-39.7-2.6-50.8-2c-11.1 0-50.1 0-64.5 1.3-14.3.6-65.1 5.9-78.8 7.8-13.7 2.6-68.4 9.8-75.5 11.7s-22.1 3.9-22.1 3.9c-5 1.1-10.2.7-15-1.3-7.2-3.2-18.9-7.8-9.1-11.1s58.6-13 73.6-15c15-2.6 62.5-9.1 82-9.8s73.6-3.9 93.8-3.9 47.5 1.3 59.9 2.6c11.7.6 19.5 1.3 22.1 2s2.6-1.3 5.2-3.9 9.8-12.4 9.8-12.4 2.6-3.9-2.6-.6c-4.6 3.2-12.4 3.2-16.3 0-3.9-3.9-5.2-7.2-7.2-5.9-4.2 2.3-8.5 4.3-13 5.9-3.9 1.3-20.8 5.2-30.6-1.3-9.8-7.2-10.4-9.8-11.7-13-1.3-4.5-2-4.5-5.2-2.6-3.3 1.9-14.3 7.8-18.9 9.8-4.1 2.4-8.9 3.3-13.7 2.6-6.5-.6-13.7-2.6-16.9-7.8-2.6-5.2-3.2-9.1-6.5-6.5s-21.5 13-31.2 15.6c-9.8 3.3-12.4 3.9-17.6 3.3-5.9-.6-11.7-3.3-13.7-6.5s-2.6-3.9-5.2-2.6c-2.6 2-9.8 7.2-15 7.2-5.2-.6-18.9-3.9-26-17.6s-2-19.5 4.6-26.7c6.5-6.5 18.2-16.9 18.2-16.9s15-11.1 19.5-13c5.2-2.6 13-7.2 18.2-7.2 7.1-.9 14.2 1.2 19.6 5.9 7.2 5.9 11.7 11.7 12.4 14.3s3.9 10.4-1.3 16.3-20.2 19.5-20.2 19.5-5.9 3.3-3.3 7.2 12.4-2.6 15.6-4.5c3.2-2.6 20.2-16.9 25.4-21.5 5.2-5.2 37.8-37.1 41-40.4 2.6-3.3 30-30 33.8-32.6 3.9-2.6 5.2-4.6 13-3.2 4.1.7 8 2 11.7 3.9 0 0 7.8 3.3 1.3 7.8-5.9 4.5-26.7 22.1-31.9 27.3s-20.2 20.2-24.7 25.4c-4.6 6.5-17.6 22.8-19.5 25.4-2 3.3-3.3 6.5 0 9.1 3.3 2 9.1 0 15-5.2s24.1-20.2 30-26.7c6.5-6.5 40.4-38.4 43.6-41.7 3.3-2.6 18.9-15 18.9-15s2-2.6 6.5-2.6 9.1.6 11.7 2l7.8 3.9s3.2 1.3-2 5.2-33.8 26-41 34.5c-7.8 8.5-22.8 25.4-28.7 32.6-5.9 7.8-9.8 11.1-5.2 16.3s9.8 3.9 12.4 3.3c6.5-3.7 12.4-8.3 17.6-13.7 6.5-6.5 26-28 28.7-30 2-2.6 9.8-9.8 9.8-9.8s1.3-3.2 5.9-3.2 6.5.6 11.1 1.3c3.9.6 13 2.6 8.5 7.8-5.2 5.2-17.6 18.2-20.2 20.8-2.6 3.2-13 15-15 18.2s-4.6 7.2-2 9.1 9.8-.6 16.3-4.6c9-4.8 17.3-10.7 24.7-17.6 8.5-7.8 22.1-20.2 22.1-20.2s2.6-5.2 12.4-3.2c9.8 2.6 12.4 3.2 12.4 3.2s7.8 2 2.6 6.5-22.1 20.2-28.6 26c-6.5 6.5-26.7 26.7-30 30.6s-13 13-9.1 13.7 33.9 5.9 40.4 7.9c5.1 3.9 28.6 11.7 30.5 13zm-336.6-162.1c-5.8-.8-11.7-.8-17.6 0-9.8 1.3-38.4 3.9-43.6 6.5s-9.8-3.3-11.7-5.2c-2.6-2-7.8-7.8-7.8-7.8s-2.6-3.3 2-4.6c5.2-1.3 34.5-5.9 43.6-7.2 9.1-.6 53.4-3.3 65.8-3.3 11.7 0 52.7 2.6 63.8 5.2 0 0 2-.6 4.6 4.6 1.2 3.3 1.9 6.9 2 10.4 0 0 2 3.9-5.2 3.3-8.5-.6-20.8-1.3-29.3-2s-28.6-2-31.9-2c-2.6 0-3.9 0-5.2 2.6-2 2.6-9.8 13-11.7 15.6s-10.4 13.7-17.6 23.5-16.3 22.1-24.7 34.5c-7.8 13-15 24.7-18.2 30-2 3.1-3.7 6.4-5.2 9.8-.6 1.3-1.3 3.3-5.2 3.3s-20.8-.6-20.8-.6-7.2.6-2.6-7.2c4.6-8.5 14.3-24.7 18.2-30.6 3.9-6.5 25.4-35.8 29.9-41 3.7-4.6 28.4-37.8 28.4-37.8z" />
  </svg>
);

/* "More coming soon" logo set. Single source of truth so desktop + mobile
   read from the same array. SVGs are filtered white for dark-theme parity
   with the integrations marquee. */
const COMING_SOON_LOGOS: Array<{ label: string; svg: React.ReactElement }> = [
  {
    label: "SAP",
    svg: (
      <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" style={{ height: 36, width: "auto" }} className={styles.logoWhite}>
        <path d="M0 6.064v11.872h12.13L24 6.064zm3.264 2.208h0.005c0.863 0.001 1.915 0.245 2.676 0.633l-0.82 1.43c-0.835 -0.404 -1.255 -0.442 -1.73 -0.467 -0.708 -0.038 -1.064 0.215 -1.069 0.488 -0.007 0.332 0.669 0.633 1.305 0.838 0.964 0.306 2.19 0.715 2.377 1.9L7.77 8.437h2.046l2.064 5.576 -0.007 -5.575h2.37c2.257 0 3.318 0.764 3.318 2.519 0 1.575 -1.09 2.514 -2.936 2.514h-0.763l-0.01 2.094 -3.588 -0.003 -0.25 -0.908c-0.37 0.122 -0.787 0.189 -1.23 0.189 -0.456 0 -0.885 -0.071 -1.263 -0.2l-0.358 0.919 -2 0.006 0.09 -0.462c-0.029 0.025 -0.057 0.05 -0.087 0.074 -0.535 0.43 -1.208 0.629 -2.037 0.644l-0.213 0.002a5.075 5.075 0 0 1 -2.581 -0.675l0.73 -1.448c0.79 0.467 1.286 0.572 1.956 0.558 0.347 -0.007 0.598 -0.07 0.761 -0.239a0.557 0.557 0 0 0 0.156 -0.369c0.007 -0.376 -0.53 -0.553 -1.185 -0.756 -0.531 -0.164 -1.135 -0.389 -1.606 -0.735 -0.559 -0.41 -0.825 -0.924 -0.812 -1.65a1.99 1.99 0 0 1 0.566 -1.377c0.519 -0.537 1.357 -0.863 2.363 -0.863zm10.597 1.67v1.904h0.521c0.694 0 1.247 -0.23 1.248 -0.964 0 -0.709 -0.554 -0.94 -1.248 -0.94zm-5.087 0.767 -0.748 2.362c0.223 0.085 0.481 0.133 0.757 0.133 0.268 0 0.52 -0.047 0.742 -0.126l-0.736 -2.37z" fill="#0FAAFF" />
      </svg>
    ),
  },
  {
    label: "Acumatica",
    svg: (
      <svg viewBox="0 0 339.3 85" xmlns="http://www.w3.org/2000/svg" style={{ height: 22, width: "auto" }} className={styles.logoWhite}>
        <path fill="#49C8F5" d="M34.6,34.2h-9.8c-5.4,0-9.8-4.4-9.8-9.8c0-5.4,4.4-9.8,9.8-9.8s9.8,4.4,9.8,9.8V34.2z M24.8-0.2C11.1-0.2,0,10.9,0,24.5c0,13.6,11.1,24.7,24.8,24.7h24.8V24.5C49.5,10.9,38.4-0.2,24.8-0.2L24.8-0.2z" />
        <path fill="#0D1130" d="M84.5,11.8h3l4.9,21.4h-13L84.5,11.8z M78.7,4.9L68.1,49.2h7.6l2.2-8.7H94l2.4,8.7h7.6L93.5,4.9H78.7z" />
        <path fill="#0D1130" d="M129.9,17.7l-0.3,6.3c0,0-5.1-0.6-7.5-0.6c-6.5,0-8,2.4-8,9.3c0,7.7,1.3,10.2,8.1,10.2c2.4,0,7.5-0.6,7.5-0.6l0.2,6.3c0,0-6.5,1.1-10,1.1c-9.8,0-13.3-4.9-13.3-17.1c0-11.5,3.9-16.3,13.5-16.3C123.6,16.5,129.9,17.7,129.9,17.7 M161.8,17.2v31.9h-7.3v-1.6c0,0-5,2.3-8.5,2.3c-9.2,0-11.1-4.7-11.1-15.9V17.2h7.3v16.6c0,6.8,0.4,9.4,5.3,9.4c2.8,0,7-1.5,7-1.5V17.2H161.8z M167.5,49.2V17.2h7.3V19c0,0,2.7-1.5,5.6-2c1.2-0.3,2.4-0.4,3.3-0.4c3.3,0.1,5.6,1,7.2,2.7c2.8-1.1,7.4-2.7,11.2-2.7c8.7,0,11.2,4.8,11.2,16v16.7h-7.3V35.3c0-8.6-0.1-12-5.3-12c-3.1,0-6.9,1.5-7,1.6c0.2,0.4,0.4,4.7,0.4,7.7v16.6h-7.3V32.7c0-6.8-0.6-9.4-5.3-9.4c-2.8,0-6.7,1.5-6.7,1.5v24.4H167.5z M243.4,40.5c0.1,2.2,0.8,2.9,2.6,3.2l-0.3,6.2c-5,0-6.7-0.8-8.6-2.2c0,0-5.5,2.2-10.5,2.2c-6.1,0-9.3-3.9-9.3-10.2c0-6.8,3.8-9.2,10.7-9.7l8-0.6v-1.9c0-2.5-1.5-3.8-4-3.8c-4.7,0-12.4,0.6-12.4,0.6l-0.3-6.2c0,0,8.3-1.4,13-1.4c7.1,0,10.9,2.9,10.9,10.8V40.5z M229.1,35.3c-2.8,0.3-4.2,1.5-4.2,4.1c0,2.7,1.2,4.2,3.3,4.2c3.4,0,8-1.3,8-1.3V35L229.1,35.3z M258.9,23.8v13.1c0,4.4,0.1,6.1,3.3,6.1c1.8,0,5.2-0.2,5.2-0.2l0.3,6.3c0,0-4.3,0.9-6.5,0.9c-7.4,0-9.6-2.8-9.6-12V23.8h-4v-6.6h4V8h7.3v9.3h8.7v6.6H258.9z" />
        <path fill="#0D1130" d="M272.6,49.2h7.3V17.2h-7.3V49.2z M272.6,12.1h7.3V4.3h-7.3V12.1z" />
        <path fill="#0D1130" d="M307.8,17.7l-0.3,6.3c0,0-5.1-0.6-7.5-0.6c-6.5,0-8,2.4-8,9.3c0,7.7,1.3,10.2,8.1,10.2c2.4,0,7.5-0.6,7.5-0.6l0.2,6.3c0,0-6.5,1.1-9.9,1.1c-9.8,0-13.3-4.9-13.3-17.1c0-11.5,3.9-16.3,13.5-16.3C301.5,16.5,307.8,17.7,307.8,17.7 M336.7,40.5c0.1,2.2,0.8,2.9,2.6,3.2l-0.3,6.2c-5,0-6.7-0.8-8.6-2.2c0,0-5.5,2.2-10.5,2.2c-6.1,0-9.3-3.9-9.3-10.2c0-6.8,3.8-9.2,10.7-9.7l8-0.6v-1.9c0-2.5-1.5-3.8-4-3.8c-4.7,0-12.4,0.6-12.4,0.6l-0.3-6.2c0,0,8.3-1.4,13-1.4c7.1,0,10.9,2.9,10.9,10.8V40.5z M322.3,35.3c-2.8,0.3-4.2,1.5-4.2,4.1c0,2.7,1.2,4.2,3.3,4.2c3.4,0,8-1.3,8-1.3V35L322.3,35.3z" />
      </svg>
    ),
  },
  {
    label: "Epicor",
    svg: (
      <svg viewBox="0 0 136 18" xmlns="http://www.w3.org/2000/svg" style={{ height: 16, width: "auto" }} className={styles.logoWhite}>
        <path d="M104.538 12.9894C104.545 13.1172 104.524 13.2448 104.478 13.3642C104.432 13.4835 104.361 13.5919 104.271 13.6824C104.18 13.7728 104.072 13.8434 103.953 13.8895C103.833 13.9355 103.706 13.9561 103.578 13.9499H92.3089C92.0618 13.9388 91.8285 13.8327 91.6576 13.6537C91.4867 13.4748 91.3915 13.2368 91.3918 12.9894V5.34676C91.3918 5.22632 91.4155 5.10705 91.4616 4.99577C91.5077 4.8845 91.5752 4.78339 91.6604 4.69822C91.7456 4.61305 91.8467 4.54549 91.958 4.4994C92.0692 4.45331 92.1885 4.42958 92.3089 4.42958H103.578C103.702 4.42371 103.826 4.44309 103.942 4.48655C104.058 4.53 104.165 4.59663 104.255 4.68239C104.344 4.76815 104.416 4.87126 104.465 4.98546C104.513 5.09966 104.538 5.22258 104.538 5.34676V12.9894ZM131.702 12.6834C134.017 12.334 135.152 10.7615 135.152 8.35931V4.73476C135.152 1.93985 133.536 0.36731 130.785 0.36731H113.579V17.9689H117.903V4.42958H129.872C129.995 4.42336 130.117 4.44282 130.231 4.48671C130.345 4.5306 130.448 4.59794 130.535 4.68438C130.621 4.77083 130.689 4.87444 130.733 4.98854C130.776 5.10264 130.796 5.22467 130.79 5.34676V7.74895C130.796 7.87103 130.776 7.99307 130.733 8.10717C130.689 8.22127 130.621 8.32488 130.535 8.41133C130.448 8.49777 130.345 8.56511 130.231 8.609C130.117 8.65289 129.995 8.67235 129.872 8.66613H119.694V12.7284H126.682L130.35 17.9697H135.635L131.702 12.6834ZM91.4359 0.36731C88.641 0.36731 87.0685 1.98322 87.0685 4.73476V13.5572C87.0685 16.3521 88.6844 17.9247 91.4359 17.9247H104.495C107.246 17.9247 108.862 16.3088 108.862 13.5572V4.73476C108.862 1.93985 107.246 0.36731 104.495 0.36731H91.4359ZM63.0025 13.5572C63.0025 16.3521 64.6184 17.9247 67.3699 17.9247H82.5693V13.9066H68.2871C68.1629 13.9067 68.04 13.8816 67.9258 13.8329C67.8116 13.7841 67.7085 13.7126 67.6228 13.6228C67.537 13.533 67.4704 13.4267 67.4269 13.3104C67.3835 13.194 67.3641 13.0701 67.3699 12.946V5.34676C67.3699 5.10351 67.4666 4.87022 67.6386 4.69822C67.8106 4.52621 68.0439 4.42958 68.2871 4.42958H82.6127V0.36731H67.3699C64.575 0.36731 63.0025 1.98322 63.0025 4.73476V13.5572ZM58.2816 0.36731H53.9174V17.9689H58.2816V0.36731ZM27.7953 0.36731V17.9689H32.1628V4.42958H44.0452C44.1708 4.41687 44.2976 4.4315 44.417 4.47248C44.5364 4.51346 44.6454 4.57981 44.7367 4.66697C44.828 4.75413 44.8993 4.86003 44.9458 4.97738C44.9922 5.09473 45.0127 5.22074 45.0058 5.34676V7.74895C45.0127 7.87496 44.9922 8.00098 44.9458 8.11833C44.8993 8.23568 44.828 8.34158 44.7367 8.42874C44.6454 8.5159 44.5364 8.58225 44.417 8.62323C44.2976 8.66421 44.1708 8.67884 44.0452 8.66613H34.2197V12.7284H44.964C47.7589 12.7284 49.3315 11.1125 49.3315 8.36094V4.73476C49.3315 1.93985 47.7156 0.36731 44.964 0.36731H27.7953ZM5.12759 0.36731C2.33268 0.36731 0.760132 1.98322 0.760132 4.73476V13.5572C0.760132 16.3521 2.37604 17.9247 5.12759 17.9247H22.0779V14.2126H6.00468C5.88416 14.2126 5.76483 14.1888 5.6535 14.1427C5.54217 14.0965 5.44102 14.0289 5.35584 13.9437C5.27066 13.8584 5.20312 13.7572 5.15708 13.6458C5.11103 13.5345 5.08739 13.4151 5.08749 13.2946V4.9974C5.08162 4.87335 5.101 4.74941 5.14446 4.63308C5.18792 4.51675 5.25455 4.41045 5.34031 4.32064C5.42607 4.23082 5.52917 4.15936 5.64337 4.11058C5.75758 4.0618 5.88049 4.03672 6.00468 4.03685H17.2727C17.4005 4.03066 17.5281 4.05126 17.6475 4.09734C17.7668 4.14342 17.8752 4.21395 17.9657 4.30441C18.0561 4.39488 18.1267 4.50326 18.1727 4.62261C18.2188 4.74195 18.2394 4.86962 18.2332 4.9974V6.61331C18.2336 6.7356 18.2083 6.85661 18.159 6.96851C18.1096 7.0804 18.0374 7.1807 17.9468 7.26291C17.8563 7.34512 17.7495 7.40741 17.6334 7.44575C17.5172 7.48409 17.3944 7.49763 17.2727 7.48549H6.88177V11.1976H18.1907C20.9856 11.1976 22.5581 9.53749 22.5581 6.83013V4.73476C22.5581 1.93985 20.9422 0.36731 18.1907 0.36731H5.12759Z" fill="#025064" />
      </svg>
    ),
  },
  {
    /* Infor's brand identity is the red square with the white wordmark inside.
       Skip .logoWhite (would collapse box + text to a single white square)
       and let the original red+white render against the dark background. */
    label: "Infor",
    svg: (
      <svg viewBox="0 0 402.78668 371.70667" xmlns="http://www.w3.org/2000/svg" style={{ height: 32, width: "auto" }}>
        <g transform="matrix(1.3333333,0,0,-1.3333333,0,371.70667)">
          <g transform="scale(0.1)">
            <path fill="#d71e25" d="M 2787.85,0 H 0 V 2787.83 H 2787.85 V 0" />
            <path fill="#ffffff" d="m 248.832,1730.91 h 176.379 v 215.05 L 248.832,1825.11 Z m 1948.898,-665.6 h 176.37 v 232.43 c 0,114.03 16.48,210.48 150.52,210.48 25.86,0 50.55,-4.7 75.26,-11.75 v 154.83 c -179.77,7.72 -225.78,-64.47 -225.78,-64.47 v 49.95 h -176.37 z m -386.58,141.09 c -88.23,0 -141.1,58.81 -141.1,144.64 0,85.85 52.87,144.63 141.1,144.63 88.18,0 141.11,-58.78 141.11,-144.63 0,-85.83 -52.93,-144.64 -141.11,-144.64 m 0,444.48 c -176.41,0 -317.5,-117.57 -317.5,-299.84 0,-182.25 141.09,-299.83 317.5,-299.83 176.34,0 317.48,117.58 317.48,299.83 0,182.27 -141.14,299.84 -317.48,299.84 m -647.34,-585.57 h 176.38 v 430.36 h 130.51 v 141.11 h -130.51 v 90.54 c 0,75.25 28.21,92.9 84.65,92.9 21.17,0 37.61,-4.7 57.61,-11.76 l 8.25,149.33 c -32.91,8.24 -68.23,10.59 -103.49,10.59 -191.66,0 -223.4,-105.81 -223.4,-230.47 z m -644.455,0 h 176.379 v 284.55 c 0,74.09 14.106,145.81 105.832,145.81 90.547,0 91.731,-84.65 91.731,-150.51 v -279.85 h 176.363 v 315.12 c 0,152.89 -32.94,270.48 -211.656,270.47 -139.914,-0.02 -169.328,-66.62 -169.328,-66.62 v 52.5 H 519.355 Z m -270.523,-0.02 h 176.383 v 571.48 H 248.832 v -571.48" />
          </g>
        </g>
      </svg>
    ),
  },
];

export default function TallyMcpPage() {
  return (
    <div className={styles.page}>
      <SiteNav />

      <main className={styles.main}>
        <div className={styles.inner}>
          {/* WebGL shader — same project as the landing-page hero so the
              two heroes share a visual language. Sits behind all hero copy. */}
          <div className={styles.shaderLayer} aria-hidden="true">
            <UnicornShader project="hNoUYN2AHKFq4LxKJyNV" />
          </div>
          <div className={styles.scrim} aria-hidden="true" />

          {/* Eyebrow pill — same component shape as the landing hero's
              "AI agents for AR" eyebrow (gradient-tile icon + 14px label). */}
          <div className={styles.eyebrow}>
            <span className={styles.eyebrowIconWrap} aria-hidden="true">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src="/FLUpFZ3WewYoIhp88GA3QqCv9A.svg"
                alt=""
                width={12}
                height={12}
                className={styles.eyebrowIcon}
              />
            </span>
            <span className={styles.eyebrowText}>
              Free · Setup in under 30 minutes
            </span>
          </div>

          {/* Headline. Three responsive line breaks let the typographic
              rhythm match the old page: 2 lines on desktop, 3 on mobile. */}
          <h1 className={styles.h1}>
            Connect your Tally{" "}
            <br className={styles.brMobile} />
            Prime{" "}
            <br className={styles.brDesktop} />
            to{" "}
            <span className={styles.brandInline}>
              <ClaudeLogo />
              Claude
            </span>{" "}
            <br className={styles.brMobile} />
            &amp;{" "}
            <span className={styles.brandInline}>
              <ChatGptLogo />
              ChatGPT
            </span>
          </h1>

          {/* Subhead */}
          <p className={styles.sub}>
            A free, copy-paste MCP server guide. Go from zero to AI-powered
            finance ops in under 30 minutes. Works with Tally Prime on
            desktop and on cloud.
          </p>

          {/* CTAs — primary mirrors the SiteNav / CTA-banner "REQUEST DEMO"
              pill: white pill, dark icon box with two stacked chevrons that
              slide on hover. Secondary stays a transparent outline pill. */}
          <div className={styles.ctaRow}>
            <a
              href="https://app.tryratio.io/sign-up"
              className={styles.ctaPrimary}
            >
              <span className={styles.ctaIconBg} aria-hidden="true">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src="/hs5ITvrZLDk3LlGJpQeTaivp4.svg"
                  alt=""
                  width={20}
                  height={20}
                  className={styles.ctaChevron}
                />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src="/hs5ITvrZLDk3LlGJpQeTaivp4.svg"
                  alt=""
                  width={20}
                  height={20}
                  className={styles.ctaChevron}
                />
              </span>
              <span className={styles.ctaLabel}>GET STARTED FOR FREE</span>
            </a>
            <a
              href="https://app.tryratio.io/sign-in"
              className={styles.ctaSecondary}
            >
              Sign in
            </a>
          </div>

          {/* Available now */}
          <p className={styles.stripLabel}>Available now</p>
          <div className={styles.availableNow}>
            <TallyPrimeLogo />
          </div>

          {/* More coming soon */}
          <p className={styles.stripLabel}>More coming soon</p>
          <div className={styles.comingSoon}>
            {COMING_SOON_LOGOS.map((l) => (
              <span key={l.label} aria-label={l.label}>
                {l.svg}
              </span>
            ))}
          </div>
        </div>
      </main>

      <SiteFooter />
    </div>
  );
}
